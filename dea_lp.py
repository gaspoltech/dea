...
def main():
    ...
    if choice == "Simulasi_LP":
        ...
        anggaran = [p_anggaran[0] +  (p_anggaran[0]*adj/100),0]
        ...
        if st.button("Klik untuk Jalankan"):
            ...
            with h1:
                ...
                newval = [element * anggaran[0] for element in outls]
                dfm['current_allocation'] = [element * p_anggaran[0] for element in dfm['value'].tolist()]
            ...
            with st.beta_expander("Selisih Lebih/Kurang Anggaran dari Realokasi",expanded=False):
                excess = anggaran[0] * (100-total) /100
            ...
            with st.beta_expander("Prediksi Kenaikan (Penurunan) PDRB dari Realokasi",expanded=False):
                gap = int(anggaran[0]) * int(growth-int(dfc.GrowthY.sum()*10000)) /10000
